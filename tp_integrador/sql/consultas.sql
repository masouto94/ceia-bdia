...
LEFT JOIN VIDEO AS v ON v.content_id = c.id
WHERE category.name = 'Bases de Datos'
  AND c.content_type = 'video'::content_type_enum
...
GROUP BY c.id, c.title, c.content_type, u.username, category.name, c.created_at
ORDER BY c.created_at DESC;
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN interaction.interaction_type = 'reaction'::interaction_type_enum THEN interaction.id END) AS reactions,
    COUNT(DISTINCT CASE WHEN interaction.interaction_type = 'share'::interaction_type_enum THEN interaction.id END) AS shares
FROM CONTENT AS c
...
-- Pregunta: que contenidos se pueden recomendar a un usuario segun sus preferencias?
SELECT
    c.id,
    c.title,
    c.content_type,
...
-- 5. Límite de resultados configurado para un usuario.
SELECT (up.content->'result_list'->>'max_results')::INT
FROM USER_PREFERENCE up
WHERE up.user_id = 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a22';

-- 6. Optimizacion: consulta de contenidos recientes por categoria.
...
-- 7. Busqueda semántica: contenidos similares a un vector de consulta.
